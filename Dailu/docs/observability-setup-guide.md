...
A step-by-step walkthrough for adding real, hand-wired observability to Dailu — first locally,
...
## Phase 1: Wire OpenTelemetry traces into `Dailu.Api` (viewed via the Aspire dashboard)

No new infrastructure yet — this just gets the SDK emitting traces somewhere you can see them.

1. Replace the full contents of `Dailu.Api/DependencyInjection.cs` with:
   ...
   `Dailu.Infrastructure.Observability.Setup`, not `Dailu.Api`.

2. In `Dailu.Api/Program.cs`, call it right after the builder is created:
...
3. Run it: `dotnet run --project Dailu.AppHost`. Wait for the console to print the Aspire
...
1. Replace `Dailu.Api/DependencyInjection.cs` with:
...
2. Run `dotnet run --project Dailu.AppHost` and hit the endpoint a few times:
...
5. Redirect the API's OTLP export from Aspire's dashboard to your own Collector. Aspire still
   orchestrates Postgres locally, so keep running through `Dailu.AppHost` — just override the
   `api` resource's environment variable in `Dailu.AppHost/AppHost.cs`:
...
6. Run everything:
   ...
   dotnet run --project Dailu.AppHost
   ```
