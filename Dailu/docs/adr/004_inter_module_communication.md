...
1. **In-Process Integration Events via MediatR/Mediator**
   - Pros:
     ...
     - Shared `Dailu.Events` project defines the event contracts
...
5. Integration event contracts live in the shared `Dailu.Events` project
...
- New reactive cross-module flows must use integration events in `Dailu.Events`
