...
using a dedicated `Dailu.MigrationsRunner` project, a self-hosted `step-ca` instance,
and short-lived SSH certificates issued to CI via a shared provisioner secret.

## Already done in the repo

- `Dailu.MigrationsRunner` - standalone console project that reuses the existing
...
- `Dailu.MigrationsRunner/Dockerfile` - a standalone Dockerfile for this image,
  separate from `Dailu.Api/Dockerfile`. This was originally a second build
...
- `Dailu.Api` only runs its own `DatabaseInitializationService` at startup when
  `ASPNETCORE_ENVIRONMENT=Development` (`Dailu.Api/Program.cs`) - local/dev
  ...
  `Dailu.MigrationsRunner` is now the *only* thing that ever applies
