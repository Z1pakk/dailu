...
Dailu is a habit-tracking application built with a modular monolith architecture on .NET 10 and an Angular SPA frontend. The backend is composed of vertical slice modules (Habit, Tag, Identity) that share infrastructure via `SharedKernel`. All modules are wired together in `Dailu.Api` and orchestrated locally via .NET Aspire.
...
src/
  shared/
    ...
    SharedInfrastructure/   — cross-cutting infrastructure shared by module Infrastructure projects
    Dailu.Events/           — cross-module domain event contracts
  ...
  modules/
    habit/
      ...
      Habit.DataTransfer/   — DTOs shared with other modules
      Habit.Integrations/   — inbound/outbound integration with other modules
    habit-entry/            — same layered structure as habit/
    habit-user/             — same layered structure; Strava/GitHub/GoogleHealth integrations
    tag/                    — same layered structure as habit/
    identity/               — same layered structure; uses ASP.NET Core Identity

  api/
    Dailu.Api/              — composition root: wires all modules, auth, OpenTelemetry

  aspire/
    Dailu.AppHost/          — .NET Aspire orchestration host

  frontend/
    Dailu.ClientApp/        — Angular SPA

  tests/
    Dailu.ArchitectureTests/ — solution-wide architecture rules
    Test.SharedKernel/       — shared test helpers/fixtures

  Dailu.Infrastructure/     — cross-cutting infrastructure (if any)
  Dailu.MigrationsRunner/   — standalone EF migrations runner
```
